...
class Application(tk.Tk):
    ...
    def on_selection_changed(self, _event: tk.Event | None = None) -> None:
        ...
        The adapter picker goes with them, and the list is frozen where it stands.
        ...
        Frozen rather than disabled, for the reason set out in `_build_list`: the
        highlight has to stay lit while it is the thing being talked about.
        """
        index = self._selected_index()
        ...
        self.listbox.bindtags(self.list_tags if not self.busy else self.frozen_tags)
    ...
    def _build_list(self, frame: ttk.Frame, *, row: int) -> None:
        ...
        # How the list is frozen while an apply is in flight: Tk's own Listbox
        # bindings come out of the tag list, and every click and arrow key that
        # could move the selection goes with them.
        #
        # Not `state=disabled`, which was tried and photographed: Tk draws no
        # selection at all on a disabled listbox — the highlight goes with the
        # rest — and the highlight is the one thing that has to go on naming the
        # row the worker was handed. Leaving the widget enabled keeps it lit.
        # The bindings on the widget itself stay, because the two they carry that
        # reach `on_apply` check `busy` for themselves.
        self.list_tags = self.listbox.bindtags()
        self.frozen_tags = tuple(tag for tag in self.list_tags if tag != self.listbox.winfo_class())
